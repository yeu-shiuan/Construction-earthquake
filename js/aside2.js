...
document.addEventListener("wheel", function (e) {
    ...
    product.forEach(function (el, index) {
        ...
        if (e.deltaY <= -0.01) {
            ...
            console.log('上')
            if (
                Math.abs(offsetBottom - document.documentElement.scrollTop) < 60
            ) {
                aside.forEach(function (el, index) {
                    el.style = "border: 0px solid black";
                });
        ...
        } else if(e.deltaY >= 0.01){
            console.log('下')
            if (
                Math.abs(el.offsetTop - (document.documentElement.scrollTop + window.innerHeight) < 60 || el.offsetTop - (document.documentElement.scrollTop) < 60)
            ) {
              // console.log(el.offsetTop - (document.documentElement.scrollTop + window.innerHeight) < 60)
                aside.forEach(function (el, index) {
                    el.style = "border: 0px solid black";
                });
                aside[index].style = "display:block";
